...
impl<'a> Analyzer<'a> {
  ...
  pub(crate) fn analyze(&self) -> Vec<lsp::Diagnostic> {
    let ctx = RuleContext::new(self.document);
...
struct RecipeDependencyCycleRule;
...
struct UndefinedIdentifierRule;

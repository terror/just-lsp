...
export const DocumentationMarkdown = ({ children }: { children: string }) => (
  <div className='text-muted-foreground [&>p:first-child]:text-foreground [&_a]:text-foreground [&_code]:text-foreground [&_h4]:text-foreground [&_h5]:text-foreground mt-4 grid min-w-0 grid-cols-1 gap-4 xl:grid-cols-2 [&_a]:underline [&_a]:underline-offset-4 [&_blockquote]:border-l-2 [&_blockquote]:pl-4 [&_code]:font-mono [&_code]:text-[0.875em] [&_h4]:mt-2 [&_h4]:font-semibold [&_h5]:font-medium [&_li+li]:mt-2 [&_ol]:list-decimal [&_ol]:pl-5 [&_pre_code]:text-sm [&_ul]:list-disc [&_ul]:pl-5 [&>*]:col-span-full [&>figure]:col-span-1'>
    <Markdown components={{ h2: 'h4', h3: 'h5', pre: CodeBlock }}>
      {children}
    </Markdown>
  </div>
);
